...
def postproccess(path_to_file):
    ...
    df, err_dict['site'] = main_loop(df, feature='site', func=site, start_idx=start_idx)
    df, err_dict['career'] = main_loop(df, feature='career', func=career, start_idx=start_idx)
    df, err_dict['educ'] = main_loop(df, feature='educ', func=educat, start_idx=start_idx)
    df, err_dict['user_descr'] = main_loop(df, feature='user_descr', func=get_user_descr, start_idx=start_idx)
    df, err_dict['it_descr'] = main_loop(df, feature='it_descr', func=it_descr, start_idx=start_idx)
    df['has_descr'] = df['user_descr'].apply(lambda x: 1 if x != '' else 0)
    df, err_dict['group_count'] = main_loop(df, feature='group_count', func=group_count, start_idx=start_idx)
